#!/bin/bash
# Submit all sampler jobs for Gaussian_d10
...
if [ "$1" = "--resume" ] && [ -n "$2" ]; then
    RESUME="$2"
    echo "Resuming jobs for Gaussian_d10 with $RESUME additional iterations..."
else
    echo "Submitting fresh jobs for Gaussian_d10..."
fi
