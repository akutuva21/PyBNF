#!/bin/bash
# Submit all sampler jobs for Degranulation
...
if [ "$1" = "--resume" ] && [ -n "$2" ]; then
    RESUME="$2"
    echo "Resuming jobs for Degranulation with $RESUME additional iterations..."
else
    echo "Submitting fresh jobs for Degranulation..."
fi
