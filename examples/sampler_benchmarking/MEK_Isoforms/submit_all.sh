#!/bin/bash
# Submit all sampler jobs for MEK_Isoforms
...
if [ "$1" = "--resume" ] && [ -n "$2" ]; then
    RESUME="$2"
    echo "Resuming jobs for MEK_Isoforms with $RESUME additional iterations..."
else
    echo "Submitting fresh jobs for MEK_Isoforms..."
fi
